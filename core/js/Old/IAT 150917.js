...
var arCategory = [{'Code':"A",'opta':"people who work in consulting"                                                           ,'optb':"other professionals (e.g., lawyers, physicians, bankers)"},
                  {'Code':"M",'opta':"people who work in consulting"                                                           ,'optb':"other professionals (e.g., lawyers, physicians, bankers)"},
                  {'Code':"E",'opta':"entrepreneurs"                                                                           ,'optb':"non-entrepreneurs"},
                  {'Code':"F",'opta':"finance professionals"                                                                   ,'optb':"other professionals"},
                  {'Code':"N",'opta':"engineers and computer scientists"                                                       ,'optb':"other professionals"},
                  {'Code':"L",'opta':"legal professionals"                                                                     ,'optb':"other professionals"},
                  {'Code':"P",'opta':"law-enforcement professionals"                                                           ,'optb':"other professionals"},
                  {'Code':"S",'opta':"sales professionals"                                                                     ,'optb':"other professionals"},
                  {'Code':"I",'opta':"professional investors (working in venture capital, asset management, or private equity)",'optb':"other professionals"},
                  {'Code':"C",'opta':"people who work in professional services (e.g., lawyers, consultants, investment bankers, public auditors)", 'optb':"other professionals"},
                  {'Code':"Q",'opta':"people with above-average IQs"                                                           ,'optb':"people whose IQs are below average"},
                  {'Code':"T",'opta':"securities traders (equities, commodities, fixed income, etc.)"                          ,'optb':"other professionals"}];
...
function checkCategory(sStr) {
	var lFound, nI;
	lFound = false;
	for (nI=0;nI<arCategory.length;nI++) {
	 if (arCategory[nI].Code==sStr) {
		lFound = true;
		break;
	 }
	}
...
function loadInstructions(stage)
{
	...
	switch(stage)
	{
		case 'one':
			sub = $("#subID").val();
			// Check that the ID 
			lFound = checkCategory(sub.substr(0,1));
			//(sub.substr(0,1).search(/[STUVWXYZ]/g)==-1)
			if ((sub === "")||!lFound) {
...
{	
	...
	if (session == 7)
	{
		...
		{
			$.get("core/survey1.html", function(data) {
				$('body').html(data);			
				sFirstChr = sub.substr(0,1);
				sOptA = "";
				sOptB = "";
				for (nI=0;nI<arCategory.length;nI++) {
				 if (arCategory[nI].Code==sFirstChr) {
					sOptA = arCategory[nI].opta;
					sOptB = arCategory[nI].optb;
					break;
				 }
				}
				$(".SubCatA").html(sOptA);
...
function checkAndPostSurvey()
{
	
	
        var sOpinion = $('input:radio[name=subOpinion]:checked').val();
	// if (typeof(sOpinion)=="undefined") {
	// 	alert("Please provide your opinion");
	// 	return;
	// }
	gOpinion = sOpinion;
	
        var sEmail = $('input:text[name=emailID]').val();
	gEmailID = sEmail;
	
        $("#exp_process").html("<img src='core/spinner.gif'>");
	...
	$.get("core/thanks.html", function(data) {
		$('body').html(data);			
	        //confCode = String.fromCharCode(Math.floor(Math.random() * ('Z'.charCodeAt() - 'A'.charCodeAt() + 1)) + 'A'.charCodeAt()) + 'C' + Math.floor(Math.random() * 10) + '5';
		confCode = randomString(4);
		resulttext = "<div style='text-align:center;padding:20px'>Thanks for participating! Your (case sensitive) confirmation code is:<h2>" + confCode + "</h2></div>";		
		//resulttext = "<div style='text-align:center;padding:20px'>Thanks for participating!</div>";		
		$("#conf_code").html(resulttext);
		});
...
function WriteFile()
{
	...
	{
		for (j=0;j<roundArray[i].length;j++)S
		{
			str += i + "," + j + ",";
	...
	//str += sFirstChr + "," + gOpinion + "," + confCode + "," + gEmailID;
	str += sFirstChr + "," + gOpinion + "," + confCode + "," + gEmailID;
